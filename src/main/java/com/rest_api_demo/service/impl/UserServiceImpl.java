...
import com.rest_api_demo.exceptions.AbstractException;
...
public class UserServiceImpl extends AbstractService<UserEntity, UserDto, String> implements UserService{
    ...
    @Override
    public PageDto<UserDto> findAll(String email, Integer page, Integer size) {
    ...
    public UserDto update(UserDto userDto, String s) {
        throw new AbstractException(HttpStatus.NOT_IMPLEMENTED.value(), "Not implemented");
    }
    ...
    public UserDto updatePassword(UserCompact userCompact, UserPrincipal userPrincipal) {
        ...
        if (userCompact.getEmail().equals(user.getId())) {
            ...
            throw new ServiceException(HttpStatus.BAD_REQUEST.value(), "Users email is not same");
    }
    ...
    private void validateRoles(Set<String> roles){
        try {
            roles.forEach(RoleType::valueOf);
